...
export class PGSBreakdown extends HTMLElement {
  ...
  async createDistributionChart(variants, containerId) {
    Debug.log(
      2,
      'PGSBreakdown',
      'Creating distribution chart with',
      variants.length,
      'variants'
    );
  ...
  async createDistributionChartOnCanvas(distributionData, canvas) {
    Debug.log(
      2,
      'PGSBreakdown',
      'Creating distribution chart from cached data'
    );
    ...
    if (
      ...
      Debug.log(
        2,
        'PGSBreakdown',
        'Calculating distribution from variants (fallback)'
      );
    ...
    Debug.log(
      ...
      'PGSBreakdown',
      'Bin counts:',
      bins.map(b => `${b.label}: ${b.count} (sum: ${b.sum.toFixed(4)})`)
    );
