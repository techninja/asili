...
export async function updateOutputManifest(updatedData) {
  ...
  if (updatedData.trait_update) {
    for (const [mondoId, traitData] of Object.entries(
      updatedData.trait_update
    )) {
      ...
      manifest.traits[mondoId] = {
        ...
        file_path:
          traitData.file_path ||
          manifest.traits[mondoId]?.file_path ||
          `${mondoId.replace(':', '_')}_hg38.parquet`,
        last_validated:
          manifest.traits[mondoId]?.last_validated || new Date().toISOString()
      };
    }
  }
        ...
        if (Object.keys(newMetadata).length > 0) {
          manifest.traits[mondoId] = {
            ...
            file_path:
              ...
              `${mondoId.replace(':', '_')}_hg38.parquet`,
            last_validated:
              manifest.traits[mondoId]?.last_validated ||
