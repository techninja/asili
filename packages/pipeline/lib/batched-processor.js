...
import { detectFormat, generateColumnExpressions, getColumnRef, generateColumnDefinitions } from './harmonization.js';
...
async function createBatches(pgsIds, maxVariantsPerBatch = null) {
    ...
    console.log(`    Using batch size: ${maxVariantsPerBatch.toLocaleString()} variants per batch`);
    ...
    for (const pgsId of pgsIds) {
        try {
            ...
            const cachedFilePath = path.join('/cache', 'pgs_files', `${pgsId}.txt.gz`);
            try {
                ...
                console.log(`    ${pgsId}: ${variantCount.toLocaleString()} variants (cached)`);
                continue;
            } catch {
...
async function processBatchWithDuckDB(batch, batchNum, traitName, totalBatches) {
    console.log(`🦆 Processing batch ${batchNum}/${totalBatches}: ${batch.length} files, ${batch.reduce((sum, f) => sum + f.variants, 0).toLocaleString()} variants`);
    
    const safeFileName = traitName.replace(':', '_');
    const batchDbPath = path.join(BATCH_DIR, `${safeFileName}_batch_${batchNum}.duckdb`);
    const batchOutputPath = path.join(BATCH_DIR, `${safeFileName}_batch_${batchNum}.parquet`);
    
    // Prepare data files and detect column counts
    const fileQueries = [];
    for (const file of batch) {
        if (!file.file_path || typeof file.file_path !== 'string') {
            console.log(`    Warning: Invalid file path for ${file.pgs_id}: ${file.file_path}`);
            continue;
        }
        
        try {
            const { columns, dataOnlyPath, dataLineCount } = await prepareFileForProcessing(file.file_path);
            ...
            if (!formatType) {
                console.log(`    Warning: Unsupported format for ${file.pgs_id} - columns: ${columns.join(', ')}`);
                continue;
            }
            ...
            console.log(`    Warning: Could not prepare ${file.pgs_id}: ${error.message}`);
        }
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        duckdb.on('close', async (code) => {
            ...
            if (code === 0 && await fs.access(batchOutputPath).then(() => true).catch(() => false)) {
                console.log(`    ✅ Batch ${batchNum}/${totalBatches} complete`);
                resolve(batchOutputPath);
            } else {
                console.log(`    ❌ Batch ${batchNum}/${totalBatches} failed (code ${code})`);
...
async function mergeBatchResults(batchFiles, traitName) {
    ...
    if (validBatchFiles.length > 10) {
        return await hierarchicalMerge(validBatchFiles, finalOutputPath, safeFileName);
    }
...
async function hierarchicalMerge(batchFiles, finalOutputPath, safeFileName) {
    ...
    // Append in groups of 10 to avoid command line length limits
    while (currentFiles.length > 10) {
        console.log(`   Level ${level}: Appending ${currentFiles.length} files into groups of 10`);
        const nextLevelFiles = [];
        
        for (let i = 0; i < currentFiles.length; i += 10) {
            const group = currentFiles.slice(i, i + 10);
            const groupOutputPath = path.join(BATCH_DIR, `${safeFileName}_level${level}_group${Math.floor(i/10)}.parquet`);
            
            await directAppend(group, groupOutputPath, `${safeFileName}_level${level}_group${Math.floor(i/10)}`);
...
async function directAppend(batchFiles, outputPath, baseName) {
    console.log(`    Direct append: ${batchFiles.length} files -> ${path.basename(outputPath)}`);
    ...
    const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'merge_parquet.py');
    ...
    try {
        ...
        console.log(`    ✓ Merged ${validFiles.length} files (${(stats.size / 1024 / 1024).toFixed(1)}MB)`);
...
export async function generateTraitPackBatched(traitName, config) {
    ...
    const missingMetadataIds = config.pgs_ids.filter(pgsId => !existingMetadata[pgsId]);
    
    let pgsMetadata = existingMetadata;
    if (missingMetadataIds.length > 0) {
        console.log(`    Found ${missingMetadataIds.length} PGS scores missing metadata`);
        const newMetadata = await collectPgsMetadata(missingMetadataIds, existingMetadata);
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        try {
            ...
            console.log(`   DEBUG: Error processing batch ${batchNum} with ${batches[i].length} files`);
            console.log(`   DEBUG: Batch files: ${JSON.stringify(batches[i].map(f => f.pgs_id))}`);
    ...
    console.log(`✓ Merge complete: ${finalFile} (${(validation.size / 1024 / 1024).toFixed(1)}MB)`);
