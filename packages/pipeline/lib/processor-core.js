...
export async function countVariantsInFile(filePath) {
    ...
    try {
        ...
        console.log(`    Warning: Could not count variants in ${filePath}: ${error.message}`);
...
export async function runDuckDBQuery(query, dbPath = null) {
    return new Promise((resolve, reject) => {
        ...
        const timeoutMinutes = query.includes('DISTINCT') && query.includes('ORDER BY') ? 15 : 5;
...
export async function validateParquetFile(filePath, minSize = 6144) {
    try {
        const stats = await fs.stat(filePath);
        
        if (stats.size < minSize) {
            throw new Error(`Generated file too small: ${stats.size} bytes (minimum ${minSize} required)`);
        }
