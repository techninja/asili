...
async function testProgressTracker() {
  ...
  const unsubscribe = tracker.subscribe(status => {
    updates.push({ ...status });
    console.log(
      `Progress: ${status.stage} - ${status.message} (${status.progress}%)`
    );
  });
  ...
  console.log(
    `✓ ProgressTracker test passed (${updates.length} updates received)`
  );
  return true;
}
...
async function testRiskCalculator() {
  ...
  if (
    ...
    console.error(
      '✗ BasicRiskCalculator test failed - invalid result structure'
    );
    return false;
  }
}
