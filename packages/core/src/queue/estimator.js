export class TimeEstimator {
  ...
  recordCompletion(traitId, duration, variantCount) {
    ...
    record.avgTime = record.times.reduce((sum, time, i) => 
      sum + (time.duration * weights[i]), 0) / totalWeight;
  }
  ...
  estimateTime(traitId, variantCount = 0) {
    ...
    if (variantCount > 0 && record.times.length > 0) {
      const avgVariants = record.times.reduce((sum, t) => sum + t.variantCount, 0) / record.times.length;
